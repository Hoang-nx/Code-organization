...
class Trainer:
    ...
    def train(self, train_loader, val_loader=None):
        ...
        logging.info(
            f"----- START TRAINING THE {self.model_name.upper()} FOR {self.n_epochs} EPOCHS -----"
        )
        ...
        logging.info(
            f"----- COMPLETED TRAINING THE {self.model_name.upper()} IN {self.total_training_time} seconds -----"
        )
    ...
    def train_epoch(self, epoch, train_loader, val_loader=None):
        ...
        base_output = (
            f"Model {self.model_name.upper()}. Local Training Epoch: {epoch} \t"
        )
        ...
        with torch.no_grad():
            for i, batch in enumerate(test_loader):
                ...
                for j in range(batch_size):
                    recon_loss[i * batch_size + j] = torch.sqrt(
                        self.criterion(
                            out[
                                j, self.config["pre_mask"] : self.config["post_mask"], :
                            ],
                            src[
                                j, self.config["pre_mask"] : self.config["post_mask"], :
                            ],
    ...
    def save_results(self) -> dict:
        ...
        df.to_csv(
            f"{self.config['result_dir']}epoch_loss_{self.model_name}.csv",
            index=False,
            header=["Epoch", "TrainingLoss", "ValidationLoss", "TrainingTime"],
        )
